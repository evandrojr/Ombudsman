...
<head>
	...
	<!-- //
		...
		var tableNames = [
				    'Avalia&ccedil;&atilde;o de resposta',
				    'Sexo',
				    'Faixa Et&aacute;ria',
				    'Escolaridade',
				    'Tipo da Mensagem',
				    'Assunto da Mensagem',
				    'Local de resid&ecirc;ncia',
		...
		var agr = '<bean:write name='indicadoresFormulaForm' property='sel_agrupamento'/>';
	
		var formula = '<%= request.getParameter("h_formula") %>';
	
		function ids2desc(pair) {
			pair = pair.split('=');
			var tableIndex = pair[0];
			var tableVal = pair[1];
			var table = tableData[tableIndex];
			return tableNames[tableIndex] + ': ' + getById(table, tableVal);
		}
	
		function ids2names(pair) {
			pair = pair.split('=');
			return tableNames[pair[0]];
		}
		...
		function getFormulaDesc(f) {
			var result = f.split(',').iterate(function(el, i) {return ids2desc(el); }).join(', ');
			result = '([' + result + '] / [Total de mensagens recebidas no ' + agrupamento.fromCharAsUnit(agr) +  ']) * 100';
			return result;
		}
	
		function getSource(f) {
			return f.split(',').iterate(function(el, i) {return ids2names(el); }).join(', ');
		}
		...
		function fillHeader() {
			...
			getRef('r_modo_exibicao').innerHTML = modos_exibicao['cd'.indexOf('<bean:write name='indicadoresFormulaForm' property='r_modo_exibicao'/>')];
			getRef('sel_agrupamento').innerHTML = agrupamento.fromChar(agr);
			getRef('txt_periodo_i').innerHTML = '<bean:write name='indicadoresFormulaForm' property='txt_periodo_i'/>';
			getRef('txt_periodo_f').innerHTML = '<bean:write name='indicadoresFormulaForm' property='txt_periodo_f'/>';
			getRef('sel_tipo_grafico').innerHTML = tipos_grafico['<bean:write name='indicadoresFormulaForm' property='sel_tipo_grafico'/>'];
			getRef('sel_fonte').innerHTML = getSource(formula);
			getRef('h_formula').innerHTML = getFormulaDesc(formula);
			getRef('txt_meta').innerHTML = '<bean:write name='indicadoresFormulaForm' property='txt_meta'/>';
...
<input type='hidden' name='op' />
	...
	<TBODY>
		<TR>
			<TD class=contentBold align=middle>&nbsp;<label tabindex="1" id="titulo">Indicadores F&oacute;rmula da Ouvidoria - <%= request.getParameter("txt_nome") %></label></TD>
		</TR>
	</TBODY>
	</TABLE>
	...
	<%@ include file="IndicadoresFormula-results-params.inc.jsp" %>
	...
	<TBODY>
		<TR>
			...
			<%@ include file="IndicadoresFormula-chart.inc.jsp" %>
	...
	<TABLE cellspacing="1" cellpadding="2" width="100%" border="0">
	<TBODY>
	<%@ include file="IndicadoresFormula-records.inc.jsp" %>
	</TBODY>
	</TABLE>
	</DIV>
...
<!-- INÍCIO DO FORMULÁRIO PARA ENVIAR O HTML DA PÃGINA -->
...
<!-- FIM DO FORMULÁRIO PARA ENVIAR O HTML DA PÃGINA -->
<script type="text/javascript" charset='iso-8859-1'>
fillHeader();
</script>
</body>
